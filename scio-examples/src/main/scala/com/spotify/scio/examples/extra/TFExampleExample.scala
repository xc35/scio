...
import com.spotify.scio.tensorflow.example.FeatureNameFeatureSpec
...
object WordCountFeatureSpec {
  ...
  val featureNameSpec: Seq[String] = FeatureNameFeatureSpec[WordCountFeatures]
...
object TFExampleExample {
  ...
  def main(cmdlineArgs: Array[String]): Unit = {
    ...
    val featureSpec = sc.parallelize(Seq(featureNameSpec))
    ...
    sc.textFile(args.getOrElse("input", ExampleData.KING_LEAR))
      ...
      .saveAsTfExampleFile(
        args("output"),
        featureSpec,
        featureSpecPath = args.getOrElse("feature-spec-path", null))
    sc.close()
  }
}
