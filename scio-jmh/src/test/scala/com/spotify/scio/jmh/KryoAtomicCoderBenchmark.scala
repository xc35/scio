...
class KryoAtomicCoderBenchmark {
  ...
  val userId = UserId(Array[Byte](1, 2, 3, 4)) // please don't use arrays outside of benchmarks
  val user = User(userId, "johndoe", "johndoe@spotify.com") // use standard coders
  val specializedUser = SpecializedUser(userId, "johndoe", "johndoe@spotify.com") // use hand-optimized coders
